...
- **完整配置管理**：入站 / 出站 / 路由（规则 + 规则集 + 全局）/ DNS（服务器 + 分流）/ 日志级别
- **协议支持**（初版，跟随 sing-box 1.13.x）：
  - 入站：Shadowsocks（2022 系加密，多用户）、VLESS + Reality + Vision
  - 出站：direct / block / Shadowsocks / SOCKS（链式代理）
...
# 或安装指定版本
bash <(curl -fsSL https://raw.githubusercontent.com/riyan6/s-ui-lite/main/install.sh) install v0.1.0

# 管理命令
bash install.sh update            # 更新面板
bash install.sh update-core 1.13.19
...
internal/service/configgen  sing-box 配置生成器（字段与官方 1.13 文档一一对应）+ 密钥工具
...
| GET | `/core/versions` | 可更新的核心版本列表（仅适配版本线 1.13.x 稳定版） |
| POST | `/core/update` | 在线下载并更新核心（自动停核心→替换→重启，失败回滚） |
| GET | `/tools/uuid`、`/tools/ss-key?method=`、`/tools/reality-keypair`、`/tools/short-id` | 密钥生成 |
...
- 入站 `config` 不含 `type/tag/listen/listen_port`（由列字段生成）；VLESS 的 `flow` 为入站级默认，客户端 `meta.flow` 可覆盖
